...
void	draw_infos(t_fractal *fractal)
{
	...
	mlx_string_put(fractal->mlx, fractal->win, 670, 50, 0xFFFFFF,
		"Max Iterations: ");
	mlx_string_put(fractal->mlx, fractal->win, 670, 70, 0xFFFFFF,
		str);
	free(str);
	str = 0;
}
...
{
	draw_infos(fractal);
	mlx_string_put(fractal->mlx, fractal->win, 20, 30, 0xFFFFFF, "Commands:");
	mlx_string_put(fractal->mlx, fractal->win, 40, 50, 0xFFFFFF, "Zoom: + -");
